// ===== Verity Detection - News Analyzer Engine =====
...
document.getElementById('download-report').addEventListener('click', () => {
    ...
    let report = `VERITY DETECTION - LEGITIMACY REPORT\n${'='.repeat(50)}\n\n`;
    ...
    report += `\n${'='.repeat(50)}\nGenerated by Verity Detection | veritydetection.com\n`;

    const blob = new Blob([report], { type: 'text/plain' });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href     = url;
    a.download = `VerityDetection_Report_${Date.now()}.txt`;
    a.click();
...
function updateFlagCount() {
    ...
    if (!btn) return;
    const count = parseInt(localStorage.getItem('veritydetection_flags') || '0');
...
document.addEventListener('click', e => {
    if (e.target.closest('#flag-content-btn')) {
        const current = parseInt(localStorage.getItem('veritydetection_flags') || '0');
        localStorage.setItem('veritydetection_flags', current + 1);
        const btn = document.getElementById('flag-content-btn');
